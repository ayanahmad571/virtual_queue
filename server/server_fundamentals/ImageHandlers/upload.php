...
if(isset($_SESSION[SESSION_CONTROLLER_NAME]) && is_numeric($_SESSION[SESSION_CONTROLLER_NAME])){
	$resp = mysqlSelect("SELECT * FROM `admin_sm_logins` l
	left join admin_sm_user_groups g on l.lum_type = g.type_id
	where 
...
order by branch_id asc limit 1
	...
	if(!is_array($getSoc)) die(json_encode(array("status"=>"0","datum"=>"Branch Not Found")));
...
if(isset($result->ids[0])){
		...
		$updateData = mysqlUpdateData("UPDATE `virtual_branch` SET `branch_logo` = 'server_fundamentals/ImageHandlers/image?id=".$result->ids[0]."' 
		WHERE `virtual_branch`.`branch_id` = ".$getSoc[0]['branch_id']."  ",true);
